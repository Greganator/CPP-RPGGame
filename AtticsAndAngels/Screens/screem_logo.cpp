...
namespace Screens
{
	...
	namespace Scenes
	{
		...
		void DrawLogoScreen()
		{
			...
			{
				DrawText(TextSubtext("A game by", 0, lettersCount), logoPositionX, logoPositionY + 80, 20, Fade(DARKGRAY, alpha));

				if (framesCounter > 20) DrawText("Greg Tinga", GetScreenWidth() / 2 -100, GetScreenHeight() / 2, 50, Fade(BLACK, alpha));
			}
		}
